...
inline std::string hex2bin(const std::string &s)
{
	...
	for (size_t i = 0; i < s.size(); i += 2) {
		char hi = lookup[(u1) s[i]];
		char lo = lookup[(u1) s[i+1]];
        if (0x80 & (hi | lo)) {
            return r = "Invalid hex data";
        }
...
inline std::string bin2hex(const std::string &s)
{
	...
	for (size_t i = 0; i < s.size(); i++) {
		char hi = s[i] >> 4;
		char lo = s[i] & 0xf;
		r.push_back(lookup[(u1) hi]);
		r.push_back(lookup[(u1) lo]);
	}
	return r;
}
...
inline std::string b64_decode(const std::string &s)
{
	...
	if (!s.size()) return r;
    if (s.size() % 4)
		return r = "Invalid base64 data size";
